...
namespace ServiceCo
{
    public partial class Commuter_ContactInformation : ContentPage
    {
        ...
        public Commuter_ContactInformation()
        {
            InitializeComponent();
        ...
        private async Task LoadUserData()
        {
            try
            {
                ...
                if (commuter != null)
                {
                    ...
                    EmailEntry.Text = commuter.Email ?? "";
                    _currentEmail = commuter.Email;
                }
                else
                {
        ...
        private async Task ShowExitConfirmation()
        {
            ...
            var exitModal = new ContactInfoExitConfirmationModalPage(modalContent, blurOverlay, this);
        ...
        private async void OnSubmitClicked(object? sender, EventArgs e)
        {
            ...
            if (isNumberChanged)
            {
                ...
                try
                {
                    bool numberExists = await _firebaseConnection.CheckMobileNumberExistsAsync(formattedNumber);
                    HideProcessing();
                    _isProcessing = false;
        ...
        private async Task NavigateToPhoneOtpVerification(string newPhoneNumber, string email, bool navigateToEmailOtp)
        {
            try
            {
                await Navigation.PushModalAsync(new Commuter_ContactOTP(
        ...
        private async Task NavigateToEmailOtpVerification(string newEmail, string phoneNumber)
        {
            try
            {
                await Navigation.PushModalAsync(new Commuter_EmailOTP(
                    currentEmail: _currentEmail ?? "",
                    newEmail: newEmail,
        ...
        private async Task ShowModal(string title, string message, string type, bool closePageOnOk)
        {
            ...
            var modal = new ContactInfoCustomModalPage(modalContent, blurOverlay, closePageOnOk, this);
    ...
    public class ContactInfoCustomModalPage : ContentPage
    {
        private Frame _modalContent;
        private Grid _blurOverlay;
        private bool _closePageOnOk;
        private Commuter_ContactInformation _parentPage;

        public ContactInfoCustomModalPage(Frame modalContent, Grid blurOverlay, bool closePageOnOk, Commuter_ContactInformation parentPage)
        {
    ...
    public class ContactInfoExitConfirmationModalPage : ContentPage
    {
        private Frame _modalContent;
        private Grid _blurOverlay;
        private Commuter_ContactInformation _parentPage;

        public ContactInfoExitConfirmationModalPage(Frame modalContent, Grid blurOverlay, Commuter_ContactInformation parentPage)
        {
            _modalContent = modalContent;
            _blurOverlay = blurOverlay;
            _parentPage = parentPage;
    ...
    public static class ContactInfoGridExtensions
    {
        public static Button WithContactInfoGridColumn(this Button button, int column)
        {
