...
namespace ServiceCo
{
    public partial class Commuter_Pin : ContentPage
    {
        ...
        public Commuter_Pin(string phoneNumber, bool isExistingUser = false)
        {
            InitializeComponent();
        ...
        private async Task SubmitPinAsync()
        {
            ...
            try
            {
                await HandleExistingUserPinVerification(pin);
            }
            catch
            {
        ...
        private async Task SubmitNewPinAsync()
        {
            ...
            try
            {
                await Navigation.PushAsync(new Commuter_Name(_phoneNumber, pin));
            }
            catch
            {
                UpdatePinStatus("An error occurred", true);
        ...
        private async Task HandleExistingUserPinVerification(string pin)
        {
            var existingUser = await _firebaseConnection.GetCommuterByPhoneNumberAsync(_phoneNumber);

            if (existingUser == null)
            {
                UpdatePinStatus("User not found. Please register first.", true);
                await Task.Delay(2000);
                await Navigation.PopAsync();
                return;
            }

            if (string.IsNullOrEmpty(existingUser.Pin) || !existingUser.IsPinSet)
            {
            ...
            if (existingUser.Pin == pin)
            {
                ...
                {
                    ShowSuccessMessage("PIN verified! Complete your profile.");
                    await Task.Delay(1500);
        ...
        {
            var lockUntilStr = Preferences.Get($"PinLock_{_phoneNumber}", "");
            if (!string.IsNullOrEmpty(lockUntilStr) && DateTime.TryParse(lockUntilStr, out var lockUntil))
            {
                ...
                else
                {
                    _lockUntil = null;
                    _failedAttempts = 0;
                    Preferences.Remove($"PinLock_{_phoneNumber}");
                    Preferences.Remove($"FailedAttempts_{_phoneNumber}");
                }
            }

            var attempts = Preferences.Get($"FailedAttempts_{_phoneNumber}", 0);
        ...
        {
            if (_lockUntil.HasValue)
                Preferences.Set($"PinLock_{_phoneNumber}", _lockUntil.Value.ToString());
            Preferences.Set($"FailedAttempts_{_phoneNumber}", _failedAttempts);
        }
        ...
        private async void OnForgotPinClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                await Task.Delay(1500);
                await Navigation.PushAsync(new Commuter_OTP(_phoneNumber, otpSelector, true));
            }
            catch
            {
        ...
        protected override void OnAppearing()
        {
            ...
            if (AppSession.IsCommuterLoggedIn() && _isExistingUser)
            {
                Dispatcher.Dispatch(() =>
                {
                    Application.Current!.MainPage = new CommuterShell();
