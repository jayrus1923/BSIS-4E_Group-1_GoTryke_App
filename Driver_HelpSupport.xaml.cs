...
namespace ServiceCo
{
    public partial class Driver_HelpSupport : ContentPage
    {
        ...
        private async void OnDeleteAccountTapped(object sender, TappedEventArgs e)
        {
            ...
            string driverId = Preferences.Get("DriverId", "");
            ...
            var existingRequest = await _firebaseConnection.GetPendingDriverDeletionRequestByDriverAsync(driverId);
        ...
        private async Task ShowAlreadyRequestedModal(DateTime requestDate)
        {
            ...
            var modalPage = new DriverHelpSupportModalPage(modalContent, blurOverlay, this);
        ...
        private async Task SubmitDeletionRequest()
        {
            ...
            {
                string driverId = Preferences.Get("DriverId", "");
                string mobileNumber = Preferences.Get("DriverMobileNumber", "");
                ...
                var deletionRequest = new DriverDeletionRequest
                {
                    ...
                    Status = "Pending",
                    RequestReason = "Driver requested account deletion"
                };

                bool success = await _firebaseConnection.SubmitDriverDeletionRequestAsync(deletionRequest);

                if (success)
                {
                    await ShowSuccessModal("Request Submitted", "Your driver account deletion request has been submitted. Admin will review and process within 3-5 business days.\n\nNote: Any pending earnings will be forfeited if not withdrawn before deletion.");
                }
                else
                {
        ...
        {
            var modalPage = (sender as Button)?.Parent?.Parent?.Parent?.Parent?.Parent as DriverHelpSupportModalPage;
            if (modalPage == null)
            {
                var btn = sender as Button;
                if (btn != null)
                {
                    var parent = btn.Parent?.Parent?.Parent?.Parent as DriverHelpSupportModalPage;
                    if (parent != null)
                    {
        ...
        private async void OnEmergencyCallTapped(object sender, TappedEventArgs e)
        {
            try
            {
                ...
                {
                    await ShowEmergencyNotSupportedModal();
                }
            }
            catch
            {
        ...
        private async Task ShowEmergencyNotSupportedModal()
        {
            isModalOpen = true;
            ...
            var modalContent = new Frame
            {
                ...
                Content = new VerticalStackLayout
                {
                    ...
                    Children =
                    {
                        ...
                        new Label
                        {
                            Text = "Phone dialing is not supported on this device. Please call 911 manually.",
                            FontSize = 14,
                            TextColor = Color.FromArgb("#6B7280"),
                            HorizontalOptions = LayoutOptions.Center,
                            HorizontalTextAlignment = TextAlignment.Center,
        ...
        {
            if (isModalOpen) return;
            ToggleFAQ("Toggle", ToggleContent, ToggleArrow, ToggleDivider);
        }

        private void OnRidePoliciesTapped(object sender, TappedEventArgs e)
        {
            if (isModalOpen) return;
            ToggleFAQ("Ride", RideContent, RideArrow, RideDivider);
        }

        private void OnPaymentCollectionTapped(object sender, TappedEventArgs e)
        {
            if (isModalOpen) return;
            ToggleFAQ("Payment", PaymentContent, PaymentArrow, PaymentDivider);
        }

        private void OnComplaintHandlingTapped(object sender, TappedEventArgs e)
        {
            if (isModalOpen) return;
        ...
        private void ToggleFAQ(string faqName, VerticalStackLayout content, Label arrow, BoxView divider)
        {
            ...
            if (currentlyExpandedFAQ != null)
            {
                ...
                {
                    case "Toggle":
                        ToggleContent.IsVisible = false;
                        ToggleDivider.IsVisible = false;
                        ToggleArrow.RotateTo(0, 250, Easing.SpringOut);
                        break;
                    case "Ride":
                        RideContent.IsVisible = false;
                        RideDivider.IsVisible = false;
                        RideArrow.RotateTo(0, 250, Easing.SpringOut);
                        break;
                    case "Payment":
    ...
    public class DriverHelpSupportModalPage : ContentPage
    {
        private Frame _modalContent;
        private Grid _blurOverlay;
        private Driver_HelpSupport _parentPage;

        public DriverHelpSupportModalPage(Frame modalContent, Grid blurOverlay, Driver_HelpSupport parentPage)
        {
    ...
    public class DriverDeletionRequest
    {
