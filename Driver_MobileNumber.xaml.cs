...
namespace ServiceCo
{
    public partial class Driver_MobileNumber : ContentPage
    {
        ...
        private async void OnNextClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                var existingDriver = await _firebaseConnection.GetDriverByPhoneNumberAsync(fullPhoneNumber);
                ...
                if (existingDriver != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(existingDriver.Pin))
                    {
                        await Navigation.PushAsync(new Driver_Pin(fullPhoneNumber, true));
                    }
                    else
                    {
                        await Navigation.PushAsync(new Driver_Name(fullPhoneNumber));
                    }
                }
                else
                {
                    ...
                    if (success)
                    {
                        ...
                        await ShowResultModal("OTP Sent", $"Verification code sent to {displayNumber}", "complete.png", true);
                        await Navigation.PushAsync(new Driver_OTP(fullPhoneNumber, _otpSelector));
                    }
                    else
                    {
