...
namespace ServiceCo
{
    public partial class Driver_OTP : ContentPage
    {
        ...
        public Driver_OTP(string phoneNumber, OtpServiceSelector otpSelector, bool isFromPinForgot = false)
        {
            InitializeComponent();
        ...
        private async Task VerifyOtpAsync()
        {
            ...
            try
            {
                ...
                _failedAttempts = 0;
                _lockUntil = null;
                await HandleDriverAfterOtpVerification();
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleDriverAfterOtpVerification()
        {
            try
            {
                var firebaseConnection = new FirebaseConnection();
                var existingDriver = await firebaseConnection.GetDriverByPhoneNumberAsync(_phoneNumber);
                ...
                if (existingDriver != null)
                {
                    ...
                    else
                    {
                        ...
                        await Task.Delay(1500);
                        await Navigation.PushAsync(new Driver_Pin(_phoneNumber, false));
                    }
                }
                else
                {
        ...
        {
            var lockUntilStr = Preferences.Get($"DriverOtpLock_{_phoneNumber}", "");
            if (!string.IsNullOrEmpty(lockUntilStr) && DateTime.TryParse(lockUntilStr, out var lockUntil))
            {
                ...
                else
                {
                    _lockUntil = null;
                    _failedAttempts = 0;
                    Preferences.Remove($"DriverOtpLock_{_phoneNumber}");
                    Preferences.Remove($"DriverOtpFailedAttempts_{_phoneNumber}");
                }
            }

            var attempts = Preferences.Get($"DriverOtpFailedAttempts_{_phoneNumber}", 0);
        ...
        {
            if (_lockUntil.HasValue)
                Preferences.Set($"DriverOtpLock_{_phoneNumber}", _lockUntil.Value.ToString());
            Preferences.Set($"DriverOtpFailedAttempts_{_phoneNumber}", _failedAttempts);
        }
