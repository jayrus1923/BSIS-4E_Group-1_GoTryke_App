...
namespace ServiceCo
{
    public partial class Commuter_FileAComplaint : ContentPage
    {
        ...
        private string _accusedId;
        private string _complainantType;
        private string _complainantId;
        private string _complainantName;
        private string _complainantPhone;

        private List<FileResult> _selectedFiles = new List<FileResult>();
        private const int MAX_FILES = 5;

        public Commuter_FileAComplaint(
            string accusedName = "",
            string accusedId = "",
            string accusedPhone = "",
            ...
            string complainantType = "commuter")
        {
            InitializeComponent();
            ...
            _complainantType = complainantType;

            _complainantId = AppSession.GetCurrentUserId();
            _complainantName = AppSession.GetCommuterName();
            _complainantPhone = AppSession.GetCommuterMobileNumber();

            Console.WriteLine($"📝 Complaint by: {_complainantName} ({_complainantId})");
            Console.WriteLine($"📋 For Ride ID: {_rideId} (FirebaseKey: {_rideFirebaseKey})");
            Console.WriteLine($"👤 Against: {accusedName} (ID: {accusedId})");

            if (!string.IsNullOrEmpty(accusedName))
                NameEntry.Text = accusedName;

            if (!string.IsNullOrEmpty(accusedPhone))
                PhoneEntry.Text = accusedPhone;

            if (!string.IsNullOrEmpty(vehicleNumber))
                VehicleNumberEntry.Text = vehicleNumber;

            if (complainantType == "driver")
            {
                InfoTitleLabel.Text = "PASSENGER INFORMATION";
                NameLabel.Text = "Passenger's Name*";
                PhoneLabel.Text = "Passenger's Phone";
                VehicleNumberStack.IsVisible = false;
                InstructionLabel.Text = "Please fill in the details of your complaint against the passenger.";

                CategoryPicker.Items.Clear();
                var driverCategories = new[]
                {
                    "Rude / Disrespectful Passenger",
                    "Refused to Pay",
                    "Harassment",
                    "Vandalism / Damage",
                ...
                foreach (var cat in driverCategories)
                    CategoryPicker.Items.Add(cat);
            }
            else
            {
                InfoTitleLabel.Text = "DRIVER INFORMATION";
                NameLabel.Text = "Driver's Name*";
                PhoneLabel.Text = "Driver's Phone";
                VehicleNumberLabel.Text = "Tricycle Plate Number";
                VehicleNumberStack.IsVisible = true;
                InstructionLabel.Text = "Please fill in the details of your complaint against the driver.";

                CategoryPicker.Items.Clear();
                var commuterCategories = new[]
                {
                    "Overcharging / Wrong Fare",
                    "Rude / Disrespectful Driver",
                    "Unsafe Driving",
                    "Vehicle Issue / Not Roadworthy",
                    "Long Route / Detour",
                    "Reckless Driving",
                    "Driver Unprofessional",
                    "Smoking / Vaping",
                    "Using Phone While Driving",
                    "Refused to Accept Ride",
                    "Harassment",
                    "Lost Item / Theft",
                    "Other"
                };
                foreach (var cat in commuterCategories)
                    CategoryPicker.Items.Add(cat);
            }

            if (!string.IsNullOrEmpty(accusedId))
                AccusedIdLabel.Text = accusedId;

            if (!string.IsNullOrEmpty(rideId))
                RideIdLabel.Text = rideId;

            ComplainantTypeLabel.Text = complainantType;
        ...
        private async void SubmitButton_Clicked(object sender, EventArgs e)
        {
            ...
            {
                await DisplayAlert("Missing", "Please enter name.", "OK");
                return;
            }
            ...
            try
            {
                ...
                var complaintObj = new Complaint
                {
                    ...
                    ComplainantType = _complainantType,
                    ComplainantPhone = _complainantPhone,

                    AccusedId = _accusedId,
                    AccusedName = name,
                    AccusedType = _complainantType == "driver" ? "commuter" : "driver",
                    AccusedPhone = PhoneEntry.Text ?? "",
                ...
                string againstType = _complainantType == "driver" ? "passenger" : "driver";
                ...
                SuccessMessageLabel.Text = $"Your complaint against {againstType} has been received.";
